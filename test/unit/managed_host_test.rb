...
class HostTest < ActiveSupport::TestCase
  ...
  test "should not save without a hostname" do
    host = ManagedHost.new
    assert !host.save
  end

  test "should fix mac address" do
    host = ManagedHost.create :name => "myhost", :mac => "aabbccddeeff"
    assert_equal "aa:bb:cc:dd:ee:ff", host.mac
  end

  test "should fix ip address if a leading zero is used" do
    host = ManagedHost.create :name => "myhost", :mac => "aabbccddeeff", :ip => "123.01.02.03"
  ...
  test "should not add domain name to hostname if it already include it" do
    host = ManagedHost.create :name => "myhost.COMPANY.COM", :mac => "aabbccddeeff", :ip => "123.1.2.3",
      :domain => Domain.find_or_create_by_name("company.com")
    assert_equal "myhost.COMPANY.COM", host.name
  end

  test "should add hostname if it contains domain name" do
    host = ManagedHost.create :name => "myhost.company.com", :mac => "aabbccddeeff", :ip => "123.01.02.03",
      :domain => Domain.find_or_create_by_name("company.com")
    assert_equal "myhost.company.com", host.name
  end

  test "should save hosts with full stop in their name" do
    host = ManagedHost.create :name => "my.host.company.com", :mac => "aabbccddeeff", :ip => "123.01.02.03",
      :domain => Domain.find_or_create_by_name("company.com")
  ...
  test "should be able to save host" do
    host = ManagedHost.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "2.3.4.3",
  ...
  test "should import facts from yaml stream" do
    h=ManagedHost.new(:name => "sinn1636.lan")
  ...
  test "should import facts from yaml of a new host" do
    assert ManagedHost.importHostAndFacts(File.read(File.expand_path(File.dirname(__FILE__) + "/facts.yml")))
  end

  test "should not save if neither ptable or disk are defined when the host is managed" do
    if unattended?
      host = ManagedHost.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "2.4.4.03",
  ...
  test "should save if neither ptable or disk are defined when the host is not managed" do
    host = ManagedHost.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "2.3.4.03",
  ...
  test "should not save if IP is not in the right subnet" do
    if unattended?
      host = ManagedHost.create :name => "myfullhost", :mac => "aabbecddeeff", :ip => "123.05.02.03",
  ...
  test "should import from external nodes output" do
    ...
    host = ManagedHost.create :name => "myfullhost", :mac => "aabbacddeeff", :ip => "2.3.4.12",
      :domain => domains(:mydomain), :operatingsystem => operatingsystems(:redhat), :subnet => subnets(:one),
  ...
  test "host cannot be created without permission" do
    ...
    host = ManagedHost.create(:name => "blahblah", :mac => "aabbecddee19", :ip => "2.3.4.09",
  ...
  test "any host can be created when permitted" do
    ...
    host = ManagedHost.create(:name => "blahblah", :mac => "aabbecddee19", :ip => "2.3.4.11",
  ...
  test "hosts can be created when hostgroups permit" do
    ...
    host = ManagedHost.create(:name => "blahblah", :mac => "aabbecddee19", :ip => "2.3.4.4",
                       :domain => domains(:mydomain),  :operatingsystem => operatingsystems(:centos5_3),
                       :architecture => architectures(:x86_64), :environment => environments(:production),
                       :subnet => subnets(:one),
  ...
  test "hosts cannot be created when hostgroups deny" do
    ...
    host = ManagedHost.create(:name => "blahblah", :mac => "aabbecddee19", :ip => "2.3.4.9",
                       :domain => domains(:mydomain),  :operatingsystem => operatingsystems(:centos5_3),
                       :architecture => architectures(:x86_64), :environment => environments(:production),
                       :subnet => subnets(:one),
                       :disk => "empty partition", :hostgroup => hostgroups(:common))
  ...
  test "a fqdn Host should be assigned to a domain if such domain exists" do
    ...
    host = ManagedHost.create :name => "host.mydomain.net", :mac => "aabbccddeaff", :ip => "2.3.04.03",
 ...
 test "custom_disk_partition_with_erb" do
  ...
  test "should save uuid on managed hosts" do
    ...
    host = ManagedHost.create :name => "myhost1", :mac => "aabbecddeeff", :ip => "2.3.4.3", :hostgroup => hostgroups(:common), :managed => true
    assert host.valid?
    assert !host.new_record?
  ...
  test "should not save uuid on non managed hosts" do
    Setting[:use_uuid_for_certificates] = true
    host = ManagedHost.create :name => "myhost1", :mac => "aabbecddeeff", :ip => "2.3.4.3", :hostgroup => hostgroups(:common), :managed => false
    assert host.valid?
    assert !host.new_record?
  ...
  test "all whitespace should be removed from hostname" do
    host = ManagedHost.create :name => "my host 1	", :mac => "aabbecddeeff", :ip => "2.3.4.3", :hostgroup => hostgroups(:common), :managed => false
    assert host.valid?
    assert !host.new_record?
  ...
  test "assign a host to a location" do
    host = ManagedHost.create :name => "host 1", :mac => "aabbecddeeff", :ip => "5.5.5.5", :hostgroup => hostgroups(:common), :managed => false
  ...
  test "update a host's location" do
    host = ManagedHost.create :name => "host 1", :mac => "aabbccddee", :ip => "5.5.5.5", :hostgroup => hostgroups(:common), :managed => false
  ...
  test "assign a host to both a location and a tenant" do
    host = ManagedHost.create :name => "host 1", :mac => "aabbccddeeff", :ip => "5.5.5.5", :hostgroup => hostgroups(:common), :managed => false

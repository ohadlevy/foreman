...
class Api::V2::SmartVariablesControllerTest < ActionController::TestCase
  ...
  test "should update smart variable" do
    put :update, { :id => lookup_keys(:one).to_param, :smart_variable => { :default_value => 8080 } }
    assert_response :success
  end
  test "should update smart variable" do
    orig_value = lookup_keys(:one).default_value
    put :update, { :id => lookup_keys(:one).to_param, :smart_variable => { :default_value => 8080 } }
    assert_response :success
    new_value = lookup_keys(:one).reload.default_value

...
function template_info(div, url) {
  ...
  $(div).load(url + "?operatingsystem_id=" + os_id + "&hostgroup_id=" + hostgroup_id + "&environment_id=" + env_id+"&provisioning="+build,
              function(response, status, xhr) {
                if (status == "error") {
                  $(div).html("<div class='alert alert-warning'><a class='close' data-dismiss='alert'>&times;</a><p>Sorry but no templates were configured.</p></div>");
                }
              });
}
...
$(function() {
  $('*[data-ajax-url]').each(function() {
    ...
    $(this).load(url, function(response, status, xhr) {
      if (status == "error") {
        $(this).closest(".tab-content").find("#spinner").html("Failed to fetch: " + xhr.status + " " + xhr.statusText);
      }
    });
  });
});

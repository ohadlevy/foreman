...
function setProxyVersion(item, response) {
  generateItem(item, response.success, response.message.version);
}

function setPluginVersion(item, response) {
  var pluginName = item.data('plugin');
  var pluginVersion = response.message.modules ? response.message.modules[pluginName] : __('Unknown');

...
function remove_widget(item){
    ...
    if (confirm(__("Are you sure you want to delete this widget from your dashboard?"))){
        $.ajax({
            ...
            url: $(item).data('url'),
            success: function(){
                notify(__("Widget removed from dashboard."), 'success');
                gridster.remove_widget(widget);
                window.location.reload();
            },
            error: function(){
                notify(__("Error removing widget from dashboard."), 'error');
            },
        });
    }
}
...
function add_widget(name){
    $.ajax({
        ...
        data: {'name': name},
        success: function(){
            notify(__("Widget added to dashboard."), 'success');
            window.location.reload();
        },
        error: function(){
            notify(__("Error adding widget to dashboard."), 'error');
        },
        dataType: 'json'
    });
}
...
function save_position(path){
    ...
    $.ajax({
        ...
        success: function(){
            notify(__("Widget positions successfully saved."), 'success');
        },
        error: function(){
            notify(__("Failed to save widget positions."), 'error');
        },
        dataType: 'json'
    });
}

...
has_one :token, :dependent => :destroy, :conditions => Proc.new {"expires >= '#{Time.now.utc.to_s(:db)}'"}, :foreign_key => :host_id
...
def self.importHostAndFacts yaml
  ...
  h ||= Host.new :name => name, :type => 'Host::Managed'
...
def self.count_habtm association
  output = {}
  counter = Host::Managed.count(:include => association.pluralize, :group => "#{association}_id")
  # returns {:id => count...}

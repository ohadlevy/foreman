...
describe('PowerStatus', () => {
  ...
  it('resolvedWithOn', () => {
    ...
    expect(
      resolvedOnBoxRendered.find('.fa.fa-power-off.host-power-status.on').length
    ).toBe(1);
    expect(
      resolvedOnBoxRendered.find('.fa.fa-power-off.host-power-status.off')
        .length
    ).toBe(0);
    expect(
      resolvedOnBoxRendered.find('.fa.fa-power-off.host-power-status.na').length
    ).toBe(0);
  ...
  it('resolvedWithOff', () => {
    ...
    expect(
      resolvedOffBoxRendered.find('.fa.fa-power-off.host-power-status.on')
        .length
    ).toBe(0);
    expect(
      resolvedOffBoxRendered.find('.fa.fa-power-off.host-power-status.off')
        .length
    ).toBe(1);
    expect(
      resolvedOffBoxRendered.find('.fa.fa-power-off.host-power-status.na')
        .length
    ).toBe(0);
